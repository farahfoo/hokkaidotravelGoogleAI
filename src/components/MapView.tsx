...
import { MapPin, Compass, RotateCcw, Clock, Navigation, RefreshCw } from 'lucide-react';
...
const STOPS: RouteStop[] = [
  {
    ...
    lat: 42.7942,
    lng: 141.6928,
    description: "The gateway to Hokkaido. Pick up the AWD rental car and gear up for the expedition.",
  ...
  },
  {
    ...
    lat: 43.1907,
    lng: 141.0037,
    description: "Historic coastal town known for its romantic preserved canal, stone warehouses, and glasswork.",
  ...
  },
  {
    ...
    lat: 43.0618,
    lng: 141.3545,
    description: "Hokkaido's vibrant capital. Famous for Odori Park, the legendary Sapporo Beer, and savory miso ramen.",
  ...
  },
  {
    ...
    lat: 43.5932,
    lng: 142.4646,
    description: "Rolling agricultural hills, magical lavender fields, and the spellbinding Shirogane Blue Pond.",
  ...
  },
  {
    ...
    lat: 42.9644,
    lng: 141.1619,
    description: "Lush hot spring town tucked inside the Shikotsu-Toya National Park canyon with pristine waters.",
  ...
  },
  {
    ...
    lat: 42.6186,
    lng: 140.8422,
    description: "An active volcano region surrounding a perfectly round, crystal clear caldera lake that never freezes.",
  ...
  },
  {
    ...
    lat: 42.7942,
    lng: 141.6928,
    description: "Last minute souvenir hunting at Aeon Mall and Royce' Chocolate World before takeoff.",
...
function getWeatherMeta(code: number) {
  if (code === 0) return { label: "Clear Sky", emoji: "☀️", color: "text-amber-500", bg: "bg-amber-500/10" };
  if (code >= 1 && code <= 3) return { label: "Partly Cloudy", emoji: "🌤️", color: "text-sky-400", bg: "bg-sky-400/10" };
  if (code === 45 || code === 48) return { label: "Foggy", emoji: "🌫️", color: "text-slate-400", bg: "bg-slate-400/10" };
  if (code >= 51 && code <= 57) return { label: "Light Drizzle", emoji: "🌧️", color: "text-teal-400", bg: "bg-teal-400/10" };
  if (code >= 61 && code <= 67) return { label: "Rainy", emoji: "🌧️", color: "text-blue-500", bg: "bg-blue-500/10" };
  if (code >= 71 && code <= 77) return { label: "Snowy", emoji: "❄️", color: "text-indigo-400", bg: "bg-indigo-400/10" };
  if (code >= 80 && code <= 82) return { label: "Showers", emoji: "🌦️", color: "text-sky-500", bg: "bg-sky-500/10" };
  if (code >= 95 && code <= 99) return { label: "Thunderstorm", emoji: "⛈️", color: "text-red-500", bg: "bg-red-500/10" };
  return { label: "Overcast", emoji: "☁️", color: "text-slate-350", bg: "bg-slate-500/10" };
}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <div className="flex items-center gap-2 text-brand-primary mb-1">
            ...
            <span className="text-xs font-bold tracking-widest uppercase text-brand-primary">Geo-itinerary Tracker</span>
          </div>
          <h2 className="font-serif text-3xl font-semibold text-brand-text">Alpine Circuit Map</h2>
          <p className="text-sm text-brand-text-muted mt-1">
            Follow the 7-day scenic route across Hokkaido's peaks, canals, and lakes.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 bg-slate-950 p-4 sm:p-6 rounded-xl border border-slate-800 relative overflow-hidden flex flex-col justify-between aspect-[4/3] min-h-[300px] sm:min-h-[400px]">
          ...
          <div className="absolute top-4 left-4 z-10 bg-slate-900/90 backdrop-blur-md border border-slate-800 rounded-lg p-2.5 text-[11px] text-slate-400">
            ...
            <span className="mt-1 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-amber-400" /> Selected Hub</span>
            <span className="mt-0.5 flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-400" /> Route Stop</span>
          </div>

          <svg className="w-full h-full overflow-visible select-none" viewBox="0 0 800 500" xmlns="http://www.w3.org/2000/svg">
            ...
            <g className="stroke-brand-primary-light stroke-[4] fill-none">
              {STOPS.map((stop, i) => {
                if (i === 0) return null;
                const prev = STOPS[i - 1];
                const isActivePath = stop.id === selectedStop.id || prev.id === selectedStop.id;
                return (
                  <motion.line 
                ...
                >
                  ...
                  {isSelected && (
                    <motion.circle 
                      ...
                      animate={{ r: [12, 22, 12], opacity: [0.3, 0.8, 0.3] }}
                  ...
                  <motion.circle 
                    ...
                    cy={stop.coordinates.y} 
                    r={isSelected ? 8 : 6}
                    className={isSelected ? "fill-amber-400" : "fill-sky-500 hover:fill-amber-300"}
                  ...
                  <text 
                    ...
                    textAnchor="middle" 
                    className={`text-[11px] font-semibold tracking-wide pointer-events-none select-none fill-slate-100 ${isSelected ? 'fill-amber-400 font-bold scale-105' : 'fill-slate-400 opacity-80'}`}
                  >
                    {stop.name === "New Chitose Airport" ? "Chitose" : (stop.name === "Chitose Departure" ? "Airport Departure" : stop.name)}
                  </text>
                  <text
                    x={stop.coordinates.x}
                  ...
                  {weatherCache[stop.id] && (
                    <g transform={`translate(${stop.coordinates.x - 24}, ${stop.coordinates.y - 42})`} className="pointer-events-none">
                      <rect 
                        width="48" 
                        height="16" 
                        rx="8" 
                      ...
                      >
                        {getWeatherMeta(weatherCache[stop.id].code).emoji} {Math.round(weatherCache[stop.id].temp)}°
                      </text>
                    </g>
                  )}
                </g>
          ...
          <div className="absolute bottom-4 right-4 flex items-center gap-1 opacity-40">
            <span className="text-[10px] font-mono tracking-wider text-slate-400">N</span>
        ...
        <div className="lg:col-span-5 flex flex-col justify-between h-full bg-brand-container-low p-6 rounded-xl border border-brand-outline-variant/30">
          <div className="space-y-5">
            ...
            <div>
              ...
              {selectedStop.travelFromPrev && (
                <div className="flex items-center gap-1.5 text-xs text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 flex-wrap sm:flex-nowrap py-1 rounded-md mb-3 mt-1 inline-flex">
                  <Navigation className="w-3.5 h-3.5 rotate-45 shrink-0" />
                  <span className="font-semibold">{selectedStop.travelFromPrev}</span>
                </div>
              )}
              ...
              <div className="bg-slate-900 text-white rounded-xl p-4 border border-slate-800 shadow-md flex flex-col justify-between space-y-3.5 my-4">
                <div className="flex items-center justify-between">
                  ...
                  >
                    <RefreshCw className={`w-3.5 h-3.5 ${Object.values(loadingWeather).some(Boolean) ? 'animate-spin' : ''}`} />
                  </button>
                </div>
                ...
                {loadingWeather[selectedStop.id] ? (
                  ...
                  <div className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-6 flex items-center gap-2">
                      ...
                      <div className="leading-none">
                        <span className="text-xl block select-none">
                          {getWeatherMeta(weatherCache[selectedStop.id].code).emoji}
                        </span>
                        <span className="block text-[9px] font-bold text-slate-400 mt-1 select-none leading-tight">
                          {getWeatherMeta(weatherCache[selectedStop.id].code).label}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="col-span-6 border-l border-slate-800 pl-3 grid grid-cols-2 gap-y-2 text-[9px] text-slate-400 font-mono leading-none">
                      <div>
                        <span className="text-slate-500 uppercase tracking-widest text-[7px] font-semibold block mb-1 font-sans">Apparent</span>
                        <span className="text-slate-200 font-bold">{Math.round(weatherCache[selectedStop.id].apparentTemp)}°C</span>
                      </div>
                      <div>
                        <span className="text-slate-500 uppercase tracking-widest text-[7px] font-semibold block mb-1 font-sans">Humidity</span>
                        <span className="text-slate-200 font-bold">{weatherCache[selectedStop.id].humidity}%</span>
                      </div>
                      <div>
                        <span className="text-slate-500 uppercase tracking-widest text-[7px] font-semibold block mb-1 font-sans">Wind</span>
                        <span className="text-slate-250 font-bold">{Math.round(weatherCache[selectedStop.id].windSpeed)} km/h</span>
                      </div>
                      <div>
                        <span className="text-slate-500 uppercase tracking-widest text-[7px] font-semibold block mb-1 font-sans">Precip</span>
                        <span className="text-slate-250 font-bold">{weatherCache[selectedStop.id].precipitation} mm</span>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-2">
                    <p className="text-[11px] text-slate-400 italic">Climate readings currently offline. Tap icon to retry.</p>
                    {weatherError && (
                      <p className="text-[9px] text-red-400 font-mono mt-1">⚠️ {weatherError}</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="space-y-3.5 pt-1">
              <div className="flex items-start gap-2.5">
                <span className="w-5 h-5 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-800 text-xs shrink-0 font-bold mt-0.5">★</span>
                <div>
                  <span className="text-xs font-bold text-brand-text block tracking-wide uppercase">Scenic Visual</span>
                  <p className="text-xs text-brand-text-muted mt-0.5">{selectedStop.scenicSpot}</p>
                </div>
              </div>

              <div className="flex items-start gap-2.5">
                <span className="w-5 h-5 rounded-full bg-teal-100 flex items-center justify-center text-teal-800 text-xs shrink-0 font-bold mt-0.5">✓</span>
                <div>
                  <span className="text-xs font-bold text-brand-text block tracking-wide uppercase">Core Activity Focus</span>
                  <p className="text-xs text-brand-text-muted mt-0.5">{selectedStop.keyActivity}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 pt-4 border-t border-brand-outline-variant/30 flex items-center justify-between">
            <span className="text-[11px] font-mono tracking-tight text-brand-text-muted">
              Lat: {selectedStop.lat.toFixed(4)}° N / Lng: {selectedStop.lng.toFixed(4)}° E
            </span>
            <button 
